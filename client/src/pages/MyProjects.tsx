...
const MyProjects = () => {
  ...
  const deleteProject = async (projectId:string) => {
    try {
      const confirm = window.confirm('Are you sure you want to delete your project?')
      if(!confirm) return;
   ...
   useEffect(()=>{
    if(session?.user && !isPending){
      ...
      navigate('/');
        toast('Plase login to view your project')
    }
   },[session?.user])

  return (
    <>
        <div className='min-h-screen bg-slate-950 px-4 md:px-16 lg:px-24 xl:px-32'>
          {loading ? (
            ...
            <div className='py-10 min-h-[80vh]'>
              ...
              <div className='flex flex-wrap gap-3.5'>
                {projects.map((project)=>(
                  <div onClick={()=>navigate(`/projects/${project.id}`)} key={project.id} className='relative group w-72 max-sm:m-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden shadow-md group hover:shadow-indigo-700/30 hover:border-indigo-800/80 transition-all duration-300'>
                    {/* Desktop mini prview */}
                    <div className='relative w-full h-40 bg-gray-900 overflow-hidden border-b border-gray-800'>
                      {project.current_code ? (
                        <iframe srcDoc={project.current_code} className='absolute top-0 w-[1200px] h-[800px] origin-top-left pointer-events-none' sandbox='allow-scripts allow-forms allow-popups' style={{ transform: 'scale(0.25)' }} />
                      ) : (
            ...
            <div className='flex flex-col items-center justify-center h-[80vh]'>
              ...
              <button onClick={()=>navigate('/')} className='text-white px-5 py-2 mt-5 rounded-md bg-indigo-500 hover:bg-indigo-600 active:scale-95 transition'>Create New</button>
            </div>
          )}
        <Footer />
        </div>
