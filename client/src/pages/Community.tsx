          ...
          {loading ? (
            ...
            <div className='py-10 min-h-[80vh]'>
              <div className='flex items-center justify-between mb-12'>
                <h1 className='text-2xl font-medium text-white'>My Published Projects</h1>
              ...
              <div className='flex flex-wrap gap-3.5'>
                {projects.map((project)=>(
                  <Link key={project.id}  to={`/view/${project.id}`}
                   target='_blank' 
                   className=' w-72 max-sm:m-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden group hover:border-indigo-800/80 transition-all duration-all duration-300'>
                    {/* Desktop mini prview */}
                    <div className='relative w-full h-40 bg-gray-900 overflow-hidden border-b border-gray-800'>
                      {project.current_code ? (
                        <iframe srcDoc={project.current_code} className='absolute top-0 w-[1200px] h-[800px] origin-top-left pointer-events-none' sandbox='allow-same-origin allow-scripts allow-popups allow-forms' style={{transform: 'scale(0.25)'}} />
                      ) : (
