...
const Pricing = () => {
  ...
  const handlePurchase = async (planId: string) => {
    try {
      if(!session?.user) return toast('Please loginto purchase credits')
  ...
  return (
    <>
      <div className='min-h-screen bg-slate-950 pd-0 px-0 md:px-16 lg:px-24 xl:px-32'>
        <div className='text-center pt-24'>
          <h2 className='text-gray-100 text-5xl font-medium'>Choose Your Plan</h2>
        ...
        <div className='pt-14 py-4 px-4 '>
          <div className='grid grid-cols-1 md:grid-cols-3 flex-wrap gap-4'>
            {plans.map((plan, idx) => (
              <div key={idx} className="p-6 bg-black/20 ring ring-indigo-950 mx-auto w-full max-w-sm rounded-lg text-white shadow-lg hover:ring-indigo-500 transition-all duration-300">
        ...
        Project <span>Creation / Revision</span>
        Consume <span>credits</span>. You can Ourachase more credits to create more projects.</p>
      <Footer />
      </div>
    </>
  )
